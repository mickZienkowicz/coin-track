...
export const FortuneSection = async ({
  currency,
  className
}: {
  currency: string;
  ...
  const [t, locale] = await Promise.all([
    getTranslations('dashboard.fortune'),
    getLocale()
  ]);

  const lastUpdateDate = format(new Date(), 'd.MM.yyyy');

  const netWorth = formatCurrency({
    cents: 2000000000,
    currency,
    language: locale as Language
  });

  const assetsCount = formatCurrency({
    cents: 2000000000,
    currency,
    language: locale as Language
  });

  const debtsCount = formatCurrency({
    cents: 2000000000,
    currency,
    language: locale as Language
  });
