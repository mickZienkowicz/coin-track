...
import { createIncome } from '@/server/income/actions/create-income';
import { revalidatePathAction } from '@/server/revalidate/actions/revalidate-path';

import { getAddIncomeFormSchema } from './utils/add-income-form-schema';

export const AddIncomeForm = ({ closeDialog }: { closeDialog: () => void }) => {
  ...
  const onSubmit = async (values: z.infer<typeof addIncomeFormSchema>) => {
    await mutate({
      data: {
        ...values,
