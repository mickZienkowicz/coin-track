...
export const PouchCard = async ({
  ...
  return (
    <Card className='pb-4! w-full'>
      ...
      <CardHeader
        className={cn(
          pouch.isDisabled && 'opacity-20',
          'mb-2 flex items-start justify-between gap-4'
        )}
      >
        <div className='flex w-full flex-col gap-1.5'>
          <div className='flex w-full justify-between gap-1.5'>
            <h3 className='text-xl font-semibold'>{pouch.name}</h3>
      ...
      <CardContent
        className={cn(pouch.isDisabled && 'opacity-20', 'flex flex-col gap-4')}
      >
      ...
      <CardFooter className='pt-4! flex w-full items-center justify-end gap-4 border-t border-sidebar-border'>
        <EditPouchDialog pouch={pouch} currency={currency} />
